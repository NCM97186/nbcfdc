...
class Cities extends ControllerBase {
  ...
  public function display($country) {
    ...
    return [
      '#title' => $this->t('Visitors from') . ' ' . $this->t($country),
      'visitors_date_filter_form' => $form,
      'visitors_table' => [
        '#type'  => 'table',
        '#header' => $header,
        '#rows'   => $this->getData($header, $country),
      ],
   ...
   *   The name ofthe country.
   *
   * @return array
   *   Array representing the table content.
   */
  protected function getData(array $header, $country) {
    ...
    foreach ($results as $data) {
      ...
      $visitors_city_url = Url::fromRoute('visitors.city_hits', ["country" => $country, "city" => $data->visitors_city]);
